...
/**
 * Created by c_rcmiguel on 9/20/2016.
 */
public class TextViewEx extends TextView {
    ...
    private String[] lineAsWords,jinde;
    ...
    public void setText(boolean wrap) {
        wrapEnabled = wrap;
        //super.setText(st);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int red = 0;
        search = new String[Constants.search_string.split(" ").length];
        str = new String[Constants.search_string.split(" ").length];
        int z = 0;
        for (String s : search) {
            str[z] = "";
            search[z] = Constants.search_string.split(" ")[z++];
        }
        ...
        int soji = -1;
        jinde= getText().toString().trim().split(" ");
        ...
        for (int i = 0; i < blocks.length && lines <= maxLines; i++) {
            ...
            for (int j = 0; j < lineAsWords.length; j++) {
                ...
                soji++;
                //JAKE ALVIN JHUN WAS HERE - Search Highlighting goes here Nov 2016

                paint.setColor(getCurrentTextColor());

                if (!Constants.search_string.equals("")) {//check if search string is not ""


                    if ((str.length < 2 && word.toLowerCase().contains(Constants.search_string.toLowerCase()))) {//sethighlight
                        paint.setColor(Color.RED);
                    } else {

                        if (jinde.length >= soji + str.length) {
                            ...
                            for (int x = 0; x < str.length; x++) {
                                if (!str[x].toLowerCase().contains(search[x].toLowerCase())) {
                                    jin = false;
                                }
                            }
